...
#   # local6
#   with tf.variable_scope('local6', reuse=tf.AUTO_REUSE) as scope:
...
#   # local7
#   with tf.variable_scope('local7', reuse=tf.AUTO_REUSE) as scope:
#     weights = _variable_with_weight_decay('weights', shape=[48, 24],
#                                           stddev=0.04, wd=0.004)
...
#   # local8
#   with tf.variable_scope('local', reuse=tf.AUTO_REUSE) as scope:
#     weights = _variable_with_weight_decay('weights', shape=[24, 12],
#                                           stddev=0.04, wd=0.004)
...
def train(total_loss, global_step, train_acc, val_acc, test_acc):
...
def accuracy(labels, logits):
    ...
    corr_predict = tf.equal(pred, int_label)
    float_pred = tf.cast(corr_predict, tf.float32)
    acc = tf.reduce_mean(float_pred)
    return acc
